...
it("renders with correct grid template columns", () => {
  const { container } = render(<Grid gridTemplateColumns="repeat(3, 1fr)" />);
...
it("renders with correct width and height", () => {
  const { container } = render(<Grid width={1 / 2} height="100px" />);
...
it("renders with correct CSS", () => {
  const { container } = render(<Grid css={{ overflow: "hidden" }} />);
