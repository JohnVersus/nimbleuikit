...
it("renders correctly", () => {
    const { getByText } = render(_jsx(Box, { p: 3, children: "Hello, world!" }));
...
it("renders as a button", () => {
    const { getByRole } = render(_jsx(Box, { as: "button", p: 3, children: "I'm a Box as a button!" }));
    const button = getByRole("button");
    expect(button).toBeTruthy();
});
it("renders as a text", () => {
    const { getByText } = render(_jsx(Box, { as: "p", p: 3, children: "I'm a Box as a text!" }));
    const text = getByText("I'm a Box as a text!");
    expect(text).toBeTruthy();
});
it("renders as an input", () => {
    const { getByPlaceholderText } = render(_jsx(Box, { as: "input", p: 3, placeholder: "I'm a Box as a input!" }));
    const input = getByPlaceholderText("I'm a Box as a input!");
    expect(input).toBeTruthy();
});
it("has correct border", () => {
    const { getByText } = render(_jsx(Box, { p: 3, children: "I'm a Box with a border!", borderWidth: "2px", borderColor: "black", borderStyle: "solid", borderRadius: "5px" }));
...
it("responds to hover", () => {
    const { getByText } = render(_jsx(Box, { p: 3, children: "I'm a Box with custom CSS!", css: {
